...
def parse_xml(content):
    try:
        if content:
            return ET.fromstring(content.encode("utf-8"))  # Convert string to bytes
        return None
...
def fetch_excel_file(file_path):
    try:
        if not os.path.exists(file_path):
            print(f"[AUS] File not found: {file_path}")
            return None

        size = os.path.getsize(file_path)
        # print(f"[AUS] Excel size: {size} bytes")

        # Pandas auto-detects engine (xls / xlsx / xlsb)
        df = pd.read_excel(file_path, sheet_name=0)
        return df

    except Exception as e:
        print(f"[AUS] Failed to read Excel via pandas: {e}")
        return None
...
def fetch_and_search(search_name=None, threshold=80, search_country=None, search_dob=None, search_address=None, search_type=None, search_number=None, search_email=None):
    ...
    sheet = workbook if workbook is not None else None
    ...
    if(search_type == "INDIVIDUAL"):
                ...
                if search_name and not search_address and not search_country and not search_number:
                    for row in rows:
                        ...
                        if  search_name and name:
                            score=fuzz.ratio(search_name.lower(),name.lower())
                            if score>=threshold:
                ...
                if sdn_type_element is not None and sdn_type_element.text.lower() == search_type.lower():
                    ...
                    if search_name is not None:
                        ...
                        for full_name in names_to_check:
                            temp_score = fuzz.ratio(search_name.lower(), full_name.lower())
        ...
        try:
            if sheet is not None:
                ...
                for _, row in sheet.iterrows():
                    ...
                    # ---------- SEARCH TYPE ----------
                    value_type = row.iloc[2]
                    if not (isinstance(value_type, str) and value_type.upper() == search_type):
                        continue

                    # ---------- NAMES ----------
                    names_to_check = []
                    if row.iloc[1]:
                        names_to_check.append(str(row.iloc[1]))
                    if row.iloc[3]:
                        names_to_check.append(str(row.iloc[3]))

                    # ---------- DOB ----------
                    dob_values = []
                    if row.iloc[4]:
                        dob_values.append(row.iloc[4])
                    ...
                    for dob in dob_values:
                        ...
                        if re.match(r'^\d{4}$', dob):
                            ...
                            if len(years) == 2:
                                formatted_dob_values.extend(
                                    [str(y) for y in range(int(years[0]), int(years[1]) + 1)]
                                )
                            ...
                            try:
                                formatted_dob_values.append(
                                    datetime.strptime(dob, '%d/%m/%Y').strftime('%d-%m-%Y')
                                )
                            except ValueError:
                                pass

                        elif re.search(r'[a-zA-Z]\)\s?\d{4}', dob):
                            formatted_dob_values.extend(re.findall(r'\d{4}', dob))

                        elif re.search(r'^[A-Za-z]+\s\d{4}$', dob):
                            formatted_dob_values.append(re.search(r'\d{4}', dob).group())

                        elif re.match(r'^\d{2}\s[A-Za-z.]+\s\d{4}$', dob):
                            try:
                                formatted_dob_values.append(
                                    datetime.strptime(dob.replace('.', ''), '%d %b %Y').strftime('%d-%m-%Y')
                                )
                            except ValueError:
                                pass

                    # ---------- ADDRESS ----------
                    address_parts = str(row.iloc[7]) if row.iloc[7] else ""
                    address_matched = search_address in address_parts if search_address else False

                    # ---------- DOB MATCH ----------
                    dob_matched = search_dob in formatted_dob_values if search_dob else False
                    ...
                    if search_name:
                        for full_name in names_to_check:
                            score = fuzz.ratio(search_name.lower(), full_name.lower())
                            if score >= threshold:
                                name_matched = True
                                name_score = score
                                break
                    ...
                    # ---------- NATIONALITY ----------
                    nationality = row.iloc[6]
                    country_matched = search_country == nationality if search_country else False

                    # ---------- RESULT ----------
                    if search_name and name_matched:
                        if (
                            (not search_dob or dob_matched) and
                            (not search_address or address_matched) and
                            (not search_country or country_matched)
                        ):
                            matching_results.append({
                                "Name": " ".join(names_to_check),
                                "Name Type": row.iloc[3],
                                "Alias_name": [],
                                "Designation": [],
                                "Date_of_Birth": row.iloc[4],
                                "Birth_Place": row.iloc[5],
                                "Birth State": "N/A",
                                "Nationality": nationality,
                                "Name_score": name_score,
        ...
        print(f"Time taken for fetching and processing: {elapsed_time:.2f} seconds SANction")
        ...
        try:
            for sdn_entry in ofacroot.findall(".//{https://sanctionslistservice.ofac.treas.gov/api/PublicationPreview/exports/XML}sdnEntry"):
                ...
                if sdn_type_element is not None and sdn_type_element.text.lower() == search_type.lower():
                    ...
                    Nationality_Elements = [add.text for add in nationality_element] if nationality_element else []
                    address_elements = sdn_entry.findall(".//{https://sanctionslistservice.ofac.treas.gov/api/PublicationPreview/exports/XML}addressList/{https://sanctionslistservice.ofac.treas.gov/api/PublicationPreview/exports/XML}address")
                    address_values=[]
                    if address_elements:
                        for address in address_elements:
        ...
        try:
            if sheet is not None:
                ...
                for _, row in sheet.iterrows():
                    ...
                    # -------- ENTITY TYPE CHECK --------
                    entity_type = row.iloc[2]
                    if not (isinstance(entity_type, str) and entity_type.upper() == search_type):
                        continue
                    ...
                    if search_address:
                        address_matched = search_address in address_parts
                    ...
                    if search_name:
                        for full_name in names_to_check:
                            temp_score = fuzz.partial_ratio(
                                search_name.lower(),
                                full_name.lower()
                            )
                            if search_name != full_name and 80 <= temp_score <= 100:
                                temp_score -= 10
                    ...
                    country_matched = False
                    if search_country:
                        country_matched = (search_country == nationality)
            ...
            print(f"Error processing AUS Individual data: {e}")
